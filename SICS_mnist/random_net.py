...
def main(args):
    ...
    train_dataset = datasets.MNIST(root='./MNIST/raw', train=True, download=True, transform=transform)
    # 测试数据集
    test_dataset = datasets.MNIST(root='./MNIST/raw', train=False, download=True, transform=transform)
    set_seed()
    #train_dataset = datasets.MNIST(root='./MNIST/raw', train=True, download=True, transform=transform)#Subset(mnist, indices)
    train_indx = numpy.random.permutation(numpy.arange(len(train_dataset)))[:len(train_dataset) - args.remove_n]
    ...
    for epoch in range(args.epochs):
        ...
        test_loss, test_acc = test(model=model,
                                   ...
                                   device=device,
                                   epoch=epoch)
    #torch.save(model.state_dict(), "net_epoch100.pth")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--remove_n', type=int, default=59404)
    # 设置转折点参S数
